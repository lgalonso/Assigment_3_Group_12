...
//    private String[] video_ids = {"uUKAYkQZXko", "-d-fF1Nx2bQ", "UBMk30rjy0o", "W273f7MYt2Y"};
    private String[] training_names = {"Abs workout", "Legs Workout", "Arms workout", "Full Body workout", "Shoulders and back workout"};
    private String[] training_descriptions = {"Get ready for one of the best Home Ab Workouts of your LIFE! Let's do this! An ABS workout that you can do whenever and wherever you like!! You don't need any equipment or weight.",
            "Learn How to Hit Every Muscle in your arms by combining calisthenics and dumbbells to get some insane results from this workout.", "Arms workout that will improve your upper body strength!",
    ...
    private int[] training_duration = {10, 20, 20, 30, 40};
    ...
    private int[] arms_difficulty = {2, 4, 3};
    private int[] arms_duration = {2, 2, 5};
    private int[] arms_timestamps = {236, 310, 359};
    private String[] arms_images = {"deadlift", "dumbbell_curl", "push_up"};
    ...
    public void loadResources(Context context) {
        ...
        for (int i = 0; i < video_ids.length; i++) {
            ...
            switch (i){
                ...
                case 2:
                    for (int j = 0; j < 3; j++) {
                        exercise.setName(arms_name[j]);
                        exercise.setDescription(arms_description[j]);
                        exercise.setDifficulty(arms_difficulty[j]);
                        exercise.setDuration(arms_duration[j]);
                        exercise.setVideo_id(video_ids[i]);
                        exercise.setImage(arms_images[j]);
                        exercise.setVideo_timestamp(arms_timestamps[j]);
                        exercise.setDate_interval(exercise_interval[j]);
                        switch (j){
                            case 0:
                                training.setExercise_key_1(arms_name[j]);
                                break;
                            case 1:
                                training.setExercise_key_2(arms_name[j]);
                                break;
                            case 2:
                                training.setExercise_key_3(arms_name[j]);
                                break;
                            default:
                                break;
                        }
